...
import mlflow
import mlflow.keras
from selfchatgpt.views import chatgpt_api
...
def index(request):
    return render(request, 'selfsignlanguagetochatgpt/index.html')
...
def chat(request):
    if request.method == 'POST' and request.FILES['files']:
        ...
        for idx,file in enumerate(files, start=0):
            ...
            model_uri = "models:/Sign_Signal_24/production"
        ...
        content = chatgpt_api(selectedChatResult.prompt)
    ...
    return render(request, 'selfsignlanguagetochatgpt/result.html', context)  